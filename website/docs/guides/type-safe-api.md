...
- **Each factory has its own cache.** As of v2.2 the translate cache is per-provider, so mounting two `createLocalization()` factories with overlapping descriptors but different templates is safe — neither shadows the other.
